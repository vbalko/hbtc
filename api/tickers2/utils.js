...
const util = {
	...
	processOneTicker : async (ticker) => {
		...
		try {
			const tickerHeaderDb = await model.t2hModel.findOne({symbol:ticker.symbol});
			if (tickerHeaderDb) {
				// check timestamp if update is necessary.
				if (new Date(tickerHeaderDb.lastTickerTimestamp) < new Date(ticker.timestamp)) {
					//update timestamp on header
					const oUpdateRes = await model.t2hModel.updateOne({ symbol: ticker.symbol }, { lastTickerTimestamp: ticker.timestamp });
					// if (oUpdateRes) {

					// }
					//create new record in t2
					const fromDBCr = await model.t2Model.create(util.convertToT2Data(await ticker));
					// if (fromDBCr) {
				...
				// }
				const fromDBCrData = await model.t2Model.create(util.convertToT2Data(await ticker));
				// if (fromDBCr) {
